#!/usr/bin/env python3
"""
harvest_medical.py — the medical wire: who pays the prescriber, what gets
prescribed and diagnosed as a result, what the safety data actually shows, who
approves it, and what it costs the people on the receiving end.

After "The Medical Industry", and scoped to what that section documents:
diagnosing more, overprescribing for profit, and the payment-to-prescription
link it quantifies — free meals doubling the odds of prescribing a company's
medicine, physicians paid over $5,000 prescribing brand-name drugs 30 per cent
of the time against 20 per cent for those paid nothing, drug-specific payments
tracking 58 per cent more of that drug on average, and the Myrbetriq and
Restasis figures. Then unnecessary procedures with a financial motive, drugs
approved on safety data that was thin or fabricated with regulators complicit,
and the Bayer case: clotting products known to carry HIV, sold into countries
with limited testing after a safe product already existed.

Around those it carries the machinery that produces them — trial data and what
never gets published, how approval works and who sits on the committees, the
door between regulator and industry, marketing and off-label promotion, who
writes the clinical guidelines, pricing and patents, billing and refusal of
cover, devices and implants, dependence, health data, mental health treatment,
what litigation actually costs, and what is set against it.

A wellness listicle is not this subject, and this wire is not medical advice.
Every subject term carries the words it must appear beside.
...
# --------------------------------------------------------------------------
USER_AGENT = ("Mozilla/5.0 (compatible; medical-feed/1.0; "
...
# THE SUBJECTS
#
# Who pays the prescriber, what gets prescribed and diagnosed as a result,
# what the safety data shows, who approves it, and what it costs the people
# on the receiving end.
#
# Each subject is (term, context) pairs. The term must appear AND one of its
# context words, so a wellness listicle or a symptom explainer cannot reach
# the wire. This is a feed on industry conduct, not medical advice.
# --------------------------------------------------------------------------
TOPICS = [
    ("payments", "Who pays the prescriber", [
        ("open payments", []), ("sunshine act", []), ("physician payment*", []),
        ("payments to doctors", []), ("payments to physicians", []),
        ("industry payment*", ["doctor", "physician", "prescriber", "clinician", "received"]),
        ("consulting fee", ["doctor", "physician", "pharmaceutical", "device", "paid"]),
        ("speaker fee", []), ("speaker bureau", []), ("speakers bureau", []),
        ("hospitality", ["doctor", "physician", "pharmaceutical", "meal", "declared"]),
        ("free meal", ["doctor", "physician", "representative", "prescrib*"]),
        ("gift", ["doctor", "physician", "pharmaceutical", "declared", "ban"]),
        ("disclosure", ["payment", "physician", "industry", "transfer of value"]),
        ("transfer of value", []),
    ]),
    ("prescribing", "What gets prescribed", [
        ("overprescrib*", []), ("prescribing rate", []), ("prescription rate", []),
        ("brand-name", ["prescrib*", "share", "instead of generic", "rate"]),
        ("generic", ["substitution", "switch", "prescrib*", "cheaper", "uptake"]),
        ("prescription", ["rose", "fell", "rate", "data", "audit", "inappropriate", "unnecessary"]),
        ("polypharmacy", []), ("deprescrib*", []),
        ("antibiotic", ["prescrib*", "overuse", "resistance", "unnecessary", "stewardship"]),
        ("inappropriate prescribing", []), ("prescribing guideline", []),
        ("formulary", ["decision", "listed", "removed", "influence"]),
    ]),
    ("overdiagnosis", "Diagnosing more", [
        ("overdiagnos*", []), ("overtreatment", []), ("overtreated", []),
        ("disease mongering", []), ("medicalis*", ["normal", "ordinary", "condition", "life"]),
        ("medicaliz*", ["normal", "ordinary", "condition", "life"]),
        ("diagnostic criteria", ["widened", "expanded", "lowered", "changed", "revised"]),
        ("threshold", ["lowered", "widened", "diagnostic", "treatment", "changed"]),
        ("screening", ["harm", "overdiagnos*", "false positive", "expanded", "criticis*", "evidence"]),
        ("false positive", []), ("incidental finding", []),
        ("new condition", ["defined", "recognised", "recognized", "criteria"]),
    ]),
    ("procedures", "Unnecessary procedures", [
        ("unnecessary procedure", []), ("unnecessary surgery", []), ("unneeded", ["procedure", "surgery", "test", "scan"]),
        ("low-value care", []), ("choosing wisely", []),
        ("fee-for-service", ["incentive", "volume", "criticis*", "reform"]),
        ("volume", ["procedure", "incentive", "target", "surgical"]),
        ("referral", ["self-referral", "financial interest", "incentive", "pattern"]),
        ("self-referral", []), ("imaging", ["unnecessary", "overuse", "referral", "rate"]),
        ("stent", ["unnecessary", "trial", "evidence", "rate"]),
    ]),
    ("harm", "What treatment does", [
        ("adverse event", []), ("adverse drug reaction", []), ("side effect", ["serious", "reported", "unreported", "study", "warning"]),
        ("drug death*", []), ("iatrogenic", []), ("medical error", []),
        ("pharmacovigilance", []), ("yellow card", []), ("adverse event reporting", []),
        ("black box warning", []), ("safety warning", ["drug", "device", "medicine", "issued"]),
        ("withdrawn", ["market", "drug", "device", "safety"]),
        ("recall", ["drug", "device", "implant", "batch", "medicine"]),
        ("harmed", ["patients", "thousands", "study", "treatment"]),
    ]),
    ("trials", "Trial data and what is published", [
        ("clinical trial", ["data", "unpublished", "results", "withheld", "registry", "transparency", "failed"]),
        ("trial data", []), ("unpublished", ["trial", "study", "results", "data"]),
        ("publication bias", []), ("ghostwrit*", []), ("ghost author*", []),
        ("data fabricat*", []), ("falsified data", []), ("fabricated", ["data", "study", "results", "trial"]),
        ("research misconduct", []), ("retracted", ["study", "paper", "trial", "journal"]),
        ("trial registry", []), ("results reporting", ["failure", "requirement", "missing"]),
        ("industry-funded", ["trial", "study", "research", "outcome"]),
        ("surrogate endpoint", []), ("post-marketing", ["study", "commitment", "requirement", "failed"]),
    ]),
    ("approval", "How it gets approved", [
        ("accelerated approval", []), ("conditional approval", []), ("emergency use", ["authorisation", "authorization"]),
        ("regulatory approval", ["granted", "withdrawn", "criticis*", "evidence", "delayed"]),
        ("advisory committee", ["vote", "overruled", "recommendation", "fda", "ema"]),
        ("user fee", []), ("industry fees", ["regulator", "agency", "funding"]),
        ("approved despite", []), ("evidence base", ["weak", "thin", "questioned", "approval"]),
        ("withdrew", ["approval", "authorisation", "authorization", "licence", "license"]),
        ("orphan designation", []), ("fast track", ["designation", "approval", "criticis*"]),
    ]),
    ("capture", "Regulators and the door between", [
        ("regulatory capture", []), ("revolving door", ["regulator", "agency", "pharmaceutical", "device", "fda", "ema"]),
        ("former regulator", ["joined", "hired", "industry", "pharmaceutical"]),
        ("conflict of interest", ["regulator", "adviser", "committee", "reviewer", "panel"]),
        ("funded by industry", ["regulator", "agency", "review"]),
        ("lobbying", ["pharmaceutical", "medical device", "health", "drug", "hospital"]),
        ("pharma lobby*", []), ("industry influence", ["regulator", "policy", "approval", "guideline"]),
    ]),
    ("marketing", "How it is sold", [
        ("direct-to-consumer", ["advertising", "advert", "drug", "marketing"]),
        ("drug advertising", []), ("pharmaceutical advertising", []),
        ("off-label", ["promotion", "marketing", "prescrib*", "use", "settlement"]),
        ("misleading", ["promotion", "advertising", "claims", "drug", "device"]),
        ("sales representative", ["doctor", "visit", "detailing", "access"]),
        ("detailing", ["physician", "pharmaceutical", "sales"]),
        ("patient group", ["funded", "industry", "pharmaceutical", "undisclosed"]),
        ("awareness campaign", ["funded", "industry", "pharmaceutical", "sponsored"]),
        ("key opinion leader", []),
    ]),
    ("guidelines", "Who writes the guidance", [
        ("clinical guideline", ["conflict", "industry", "panel", "revised", "criticis*"]),
        ("guideline panel", []), ("guideline committee", []),
        ("expert panel", ["conflict", "industry", "payment", "member"]),
        ("consensus statement", ["industry", "conflict", "funded"]),
        ("continuing medical education", []), ("cme", ["funded", "sponsor", "industry"]),
        ("medical journal", ["conflict", "industry", "advertising", "funding", "influence"]),
        ("peer review", ["failure", "conflict", "flawed", "journal"]),
    ]),
    ("pricing", "Price, patent and access", [
        ("drug pricing", []), ("drug price", ["rose", "increase", "cut", "negotiat*", "cap", "hike"]),
        ("price gouging", []), ("patent", ["extend*", "evergreen*", "thicket", "challenge", "expired", "drug"]),
        ("evergreen*", []), ("compulsory licence", []), ("compulsory license", []),
        ("generic entry", []), ("biosimilar", []), ("pay-for-delay", []),
        ("access to medicine", []), ("essential medicine", ["list", "access", "shortage", "price"]),
        ("shortage", ["drug", "medicine", "supply", "essential"]),
        ("out-of-pocket", ["medicine", "treatment", "cost", "patients"]),
    ]),
    ("insurance", "Billing, cover and refusal", [
        ("prior authorisation", []), ("prior authorization", []),
        ("claim denied", []), ("denial rate", ["insurer", "claim", "coverage"]),
        ("insurer", ["denied", "refused", "coverage", "profit", "investigation"]),
        ("surprise billing", []), ("medical debt", []), ("medical bankruptcy", []),
        ("upcoding", []), ("billing fraud", []), ("false claims act", []),
        ("coverage", ["denied", "refused", "restricted", "rationed", "withdrawn"]),
        ("private equity", ["hospital", "clinic", "practice", "healthcare", "nursing"]),
    ]),
    ("devices", "Devices and implants", [
        ("medical device", ["approval", "recall", "safety", "harm", "implant", "regulation"]),
        ("implant", ["failure", "recall", "harm", "registry", "removed"]),
        ("device regulation", []), ("notified body", []),
        ("mesh", ["implant", "harm", "inquiry", "banned", "litigation"]),
        ("hip replacement", ["recall", "failure", "harm"]),
        ("device registry", []), ("510(k)", []), ("predicate device", []),
    ]),
    ("dependence", "Addiction and dependence", [
        ("opioid", ["crisis", "epidemic", "prescrib*", "settlement", "marketing", "deaths"]),
        ("oxycontin", []), ("purdue", ["settlement", "sackler", "opioid"]), ("sackler", []),
        ("benzodiazepine", ["dependence", "prescrib*", "withdrawal", "long-term"]),
        ("dependence", ["prescribed", "medicine", "drug", "withdrawal", "patients"]),
        ("withdrawal", ["antidepressant", "benzodiazepine", "prescribed", "symptoms", "guidance"]),
        ("addictive", ["prescribed", "medicine", "marketed", "drug"]),
        ("prescription drug", ["death", "misuse", "diversion", "overdose"]),
    ]),
    ("contamination", "Unsafe and contaminated products", [
        ("contaminated", ["blood product", "medicine", "batch", "vial", "supply", "drug"]),
        ("infected blood", []), ("contaminated blood", []), ("factor viii", []),
        ("tainted", ["product", "medicine", "supply", "batch"]),
        ("falsified medicine", []), ("counterfeit medicine", []), ("substandard medicine", []),
        ("impurity", ["found", "recall", "carcinogen", "batch", "limit"]),
        ("manufacturing", ["violation", "breach", "inspection", "plant", "warning letter"]),
        ("warning letter", ["plant", "manufactur*", "inspection", "facility"]),
        ("continued selling", []), ("dumped", ["market", "product", "stock", "abroad"]),
    ]),
    ("global", "What is done where oversight is thinnest", [
        ("developing countries", ["sold", "dumped", "trial", "tested", "exported", "marketed"]),
        ("low-income countries", ["trial", "access", "priced", "sold", "tested"]),
        ("clinical trial", ["conducted abroad", "offshore", "outsourced", "consent"]),
        ("informed consent", ["failure", "breach", "trial", "obtained"]),
        ("ethics approval", ["missing", "breach", "trial", "granted"]),
        ("export", ["banned drug", "unapproved", "medicine", "device"]),
        ("tiered pricing", []), ("licensing agreement", ["medicine", "generic", "access", "patent pool"]),
        ("patent pool", []),
    ]),
    ("data", "Health records as an asset", [
        ("health data", ["sold", "shared", "breach", "commercial", "access", "consent"]),
        ("patient data", ["sold", "shared", "breach", "commercial", "consent"]),
        ("medical record", ["breach", "sold", "shared", "access", "leak"]),
        ("de-identified", []), ("data sharing agreement", ["health", "hospital", "patient"]),
        ("algorithm", ["clinical", "bias", "triage", "diagnostic", "approved", "audit"]),
        ("ai", ["diagnostic", "clinical", "approved", "regulator", "evidence", "hospital"]),
        ("consent", ["data", "records", "opt-out", "patients"]),
    ]),
    ("mental", "Mental health treatment", [
        ("antidepressant", ["prescrib*", "rate", "withdrawal", "trial", "efficacy", "children"]),
        ("antipsychotic", ["prescrib*", "care home", "children", "off-label", "rate"]),
        ("adhd", ["prescrib*", "diagnos*", "rate", "medication", "criteria"]),
        ("mental health", ["prescrib*", "diagnos*", "treatment", "criteria", "medicalis*", "medicaliz*"]),
        ("psychiatric", ["diagnos*", "prescrib*", "criteria", "medication", "detention"]),
        ("dsm", []), ("chemical imbalance", []),
        ("talking therapy", ["access", "waiting", "cut", "provision"]),
    ]),
    ("litigation", "What it costs them when they lose", [
        ("settlement", ["pharmaceutical", "drug", "device", "opioid", "medical", "billion"]),
        ("class action", ["drug", "device", "implant", "medical", "patients"]),
        ("fine", ["pharmaceutical", "drug company", "device", "regulator", "billion"]),
        ("whistleblower", ["pharmaceutical", "hospital", "device", "trial", "retaliation"]),
        ("fraud", ["healthcare", "medicare", "medicaid", "billing", "pharmaceutical"]),
        ("guilty plea", ["pharmaceutical", "drug", "device", "healthcare"]),
        ("corporate integrity agreement", []),
        ("public inquiry", ["infected blood", "medical", "implant", "harm", "scandal"]),
    ]),
    ("alternatives", "Prevention, and what is set against it", [
        ("prevention", ["funding", "underfunded", "shifted", "spending", "priorit*"]),
        ("public health", ["funding", "cut", "underfunded", "priorit*"]),
        ("social determinant*", []), ("health inequalit*", []),
        ("patient safety", ["campaign", "inquiry", "reform", "body"]),
        ("transparency", ["payments", "trial", "pricing", "register", "database"]),
        ("open science", []), ("data transparency", ["trial", "regulatory", "medicine"]),
        ("campaign", ["patients", "harmed", "transparency", "drug pricing", "safety"]),
        ("reform", ["approval", "regulation", "pricing", "payments", "device"]),
    ]),
]

ANCHOR = [
    # Medical-industry-conduct language. A symptom explainer or a wellness
    # listicle is not this subject; who paid the prescriber, what the trial data
    # showed, who approved it, and who was harmed, is.
    "open payments", "sunshine act", "payments to doctors", "physician payments",
    "transfer of value", "overprescribing", "overdiagnosis", "overtreatment",
    "disease mongering", "low-value care", "unnecessary procedure",
    "adverse event", "pharmacovigilance", "black box warning", "iatrogenic",
    "clinical trial data", "publication bias", "ghostwriting", "research misconduct",
    "accelerated approval", "advisory committee", "regulatory capture",
    "revolving door", "conflict of interest", "direct-to-consumer advertising",
    "off-label promotion", "key opinion leader", "clinical guideline",
    "drug pricing", "price gouging", "evergreening", "pay-for-delay",
    "compulsory licence", "access to medicine", "essential medicines",
    "prior authorisation", "prior authorization", "medical debt", "upcoding",
    "false claims act", "medical device", "notified body", "implant failure",
    "opioid crisis", "oxycontin", "benzodiazepine dependence",
    "contaminated blood", "infected blood", "falsified medicine",
    "informed consent", "health data", "patient data",
    "corporate integrity agreement", "whistleblower", "patient safety",
]

BLOCK = [
    # health-and-wellness pages: most health coverage, none of this subject
    "health tips", "wellness tips", "morning routine", "best supplements",
    "superfood", "detox", "boost your immune", "signs you may have",
    "symptoms to watch", "what causes", "home remedy", "workout plan",
    "weight loss tips", "skincare routine", "sleep hacks", "recipe",
    "doctor explains", "myths debunked", "expert reveals", "here's what happens",
    # individual and celebrity health stories
    "celebrity diagnosis", "opens up about", "shares her journey", "brave battle",
    "gofundme", "miracle recovery", "heartwarming",
    # business-page coverage with no conduct angle
    "quarterly results", "earnings beat", "share price", "market cap",
    "stock rose", "analyst upgrade", "product launch", "hiring spree",
...
DECIDED = [
    ...
    "fined", "settled", "recalled", "withdrawn", "suspended", "convicted",
    "guilty plea", "ordered", "licence revoked", "struck off", "delisted",
]
INSTITUTIONAL = [
    "world health organization", "world health organisation", "food and drug administration",
    "european medicines agency", "mhra", "therapeutic goods administration",
    "health canada", "ministry of health", "national institute for health",
    "cochrane", "bmj", "the lancet", "jama", "new england journal",
    "inspector general", "government accountability office", "national audit office",
    "auditor general", "public inquiry", "parliamentary committee", "select committee",
    "official gazette", "court filing", "warning letter",
    "peer-reviewed", "published in", "study finds", "systematic review",
    "randomised trial", "randomized trial", "cohort study", "official data",
    "government figures", "national statistics", "registry data",
]
MEASURED = [
    ...
    "per patient", "prescriptions", "odds ratio", "relative risk", "prevalence",
]
PENDING = [
...
def run(dry_run=False, fixtures=None):
    ...
    payload = {
        ...
        "languages": languages,
        "standings": [
            {"id": "official", "label": "Medicines regulators, courts & inquiries"},
            {"id": "research", "label": "Research, journals & evidence review"},
            {"id": "press", "label": "Press"},
            {"id": "rights", "label": "Patient & access organisations"},
        ],
    ...
    if dry_run:
        print("\n--dry-run: wire_medical.json not written")
        return payload
